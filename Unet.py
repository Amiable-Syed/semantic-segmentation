...
Original file is located at
    https://colab.research.google.com/drive/1aMpp2H7DYjp64KtxxB32pLh3eWMTG7ux
"""
...
ids = next(os.walk("Training/Images"))[2] # list of names all images in the given path
...
for n, id_ in tqdm_notebook(enumerate(ids), total=len(ids)):
    # Load images
    img = load_img("Training/Images/"+id_, grayscale=True)
    x_img = img_to_array(img)
    x_img = resize(x_img, (512,512, 1), mode = 'constant', preserve_range = True)
    # Load masks
    mask_id_ = id_[:-4]+"_bin_mask.png"
    mask = img_to_array(load_img("Training/ground Truth/"+mask_id_, grayscale=True))
...
callbacks = [
    ...
    ReduceLROnPlateau(factor=0.1, patience=50, min_lr=0.0001, verbose=1),
...
test_ids = next(os.walk("Test/Images"))[2] # list of names all images in the given path
print("No. of images = ", len(test_ids))
# tqdm is used to display the progress bar
for n, id_ in tqdm_notebook(enumerate(test_ids), total=len(test_ids)):
    # Load images
    img = load_img("Test/Images/"+id_, grayscale=True)
    x_img = img_to_array(img)
    x_img = resize(x_img, (512,512, 1), mode = 'constant', preserve_range = True)
    # Load masks
    mask_id_ = id_[:-4]+"_bin_mask.png"
    mask = img_to_array(load_img("Test/ground Truth/"+mask_id_, grayscale=True))
    mask = resize(mask, (512, 512, 1), mode = 'constant', preserve_range = True)
    # Save images
    X[n] = x_img/255.0
    y[n] = mask/255.0
...
plot_sample(X_train, y_train, x_test, preds_test_t, ix=14)
